...
router.post('/hafalan', async (req, res) => {
    try {
        ...
        const query = `INSERT INTO memorization_records (name, surah_from, ayat_from, surah_to, ayat_to, status) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`;
...
// Get single record
router.get('/hafalan/:id', async (req: any, res: any) => {
    try {
        const { id } = req.params;
...
router.put('/hafalan/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query('UPDATE memorization_records SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *', [status, id]
    );
        res.json({ status: 'success', data: result.rows[0] });
    } catch (err: any) {
        res.status(500).json({ status: 'error', message: err.message });
